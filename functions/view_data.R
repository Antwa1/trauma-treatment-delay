...
view_data <- function(dataset){
  ...
  model <- glm(OFI_delay ~ Total_GCS + Gender + survival_after_30_days + Highest_care_level + Respiratory_rate + Systolic_blood_pressure + dt_ed_first_ct + pre_intubated + ISS + pt_age_yrs, data = New.subset, family = binomial)
