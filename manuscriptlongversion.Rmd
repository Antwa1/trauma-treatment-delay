...
treatment. *Aim:* This retrospective cohort study aims to identify
patient level factors that are associated with receiving delayed
treatment among trauma patients.
<!-- The first sentence is a bit complicated. Try to simplify, break 
into multiple sentences --> *Material and Methods:* This study uses the
database of Karolinska university hospital, the equivalent of a level 1
trauma center, to run a logistical regression linking the outcome,
delayed treatment, to variables, observed patient level factors.
Patients treated for trauma at the hospital between 2012 and 2022 were
included, with ISS\>9 and/or trauma code activation. Exclusion criterias
were not being screened for OFI and missing values in any of the
variables, age, gender, ISS, SBP, RR, GCS and number of injuries.
*Results:* Higher ISS and RR was linked to higher rates of delay in
treatment (ISS, OR 1.03, 95% CI 1.01-1.06)(RR, OR 1.04, 95% CI
1.01-1.08), while a lower GCS also was linked to delayed treatment (OR
0.93, 95% CI 0.87-1.00). Number of injuries showed a correlation to
higher rates of delay, but not when adjusted for. The remaining
variables showed no correlation. *Conclusion:* Our study suggests that
it is mostly more severely injured and less awake patients that receive
delayed treatment.
...
vanligaste. *Syfte:* Denna retrospektiva kohortstudie syftar till att
identifiera patientfaktorer kopplade till försenad vård bland
traumapatienter. *Material och Metod:* Denna studie använder Karolinska
universitetssjukhusets databas, motsvarande ett traumacenter på nivå 1,
för att köra en logistisk regression som kopplar utfallet, försenad
vård, till variabler, observerade patientnivåfaktorer. Patienter som
behandlades för trauma på sjukhuset mellan 2012 och 2022 inkluderades,
med ISS\>9 och/eller traumakodaktivering. Exklusionskriterier var om
patient inte screenades för ofi samt saknade värden i de använda
variablerna, ålder, kön, ISS, SBT, AF, GCS och antal skador. *Resultat:*
...
a major cause of death around the world [@WHO_2021].
<!-- The sentence below is long and overly complicated. Are you saying 
that trauma systems is an important part of healthcare and its implementation
has reduced mortality and morbidity?--> Within modern hospital care, the
...
means, such as hospital care, patient follow up and prevention programs
[@Choi_2021].

## Trauma globally

```{=html}
<!-- I suggest you name this section something else, like "Trauma globally" 
or something like that. Also, divide the second paragraph into two or 
three paragraphs-->
```
...
[@Moran_2018; @Celso_2006; @MacKenzie_2006; @Haas_2012]. During the last
three decades, the introduction of trauma systems has contributed to
lowering the incidence of preventable death. This is attributed to the
improvements in care for acute brain injuries and bleeding control. The
incidence of late death because of sepsis and multiple organ failure has
also been lowered, possibly a result of better and earlier resuscitation
[@Asensio_2008]. Functioning trauma systems saves lives in the hospital,
but their work outside of the hospital is just as important. Data
collection from injured patients, such as mechanism of injury or
mortality, are essential for creating databases that can be used for
research. In turn, that research can be used for planning injury
prevention programs that target the most common injuries in the most
efficient ways, e.g. teen drivers, children, specific occupations etc.
These injury prevention programs can be planned on trauma center,
...
<!-- Consider renaming this section as "Opportunities for improvement" -->
...
```{=html}
<!-- Make sure the aim here is exactly the same as in the abstract. Also,
I suggest that your aim is to determine the associations between different 
patient level factors and receiving delayed treatment -->
```
...
```
Table 1 shows the characteristics of the group used in the logistic
...
In the following table<!-- See comments above-->, the results from the
logistical regression analysis is displayed.

The odds ratio is calculated both unadjusted and adjusted for other
variables<!-- What does "other" mean here?-->, as well as a 95%
confidence interval. The p value is also shown.

<!-- Use "associated" instead of correlated or linked etc-->
...
correlation of higher number of injuries having higher incidence of
delayed treatment unadjusted (OR 1.06, 95% CI 1.02-1.10), but that
correlation was not seen when adjusted for. The three remaining
variables, age, gender and SBP, showed no correlation with the observed
outcome.
...
suggests that a more heavily injured and less awake patient is more
...
When looking at the patient demographics, it is clear that the majority
admitted to trauma care are men, with it being slightly higher in the
delay to treatment group. However, not high enough to be statistically
significant. The fact that gender did not show an increased odds ratio
should be seen as a positive sign. It suggests that among trauma
patients, there is no systematic delay based on this patient level
factor.
...
<!-- Do not use contractions, i.e. can't, doesn't etc --> present in
this study, though it is something that we cannot control in this study.
...
```{=html}
<!-- Not so sure about this section. What do you mean that the logistic
regression assumes that the data is binomial? Do you mean that it requires 
a binary outcome? I think you discuss potential limitations well enough in 
the previous paragraphs, so you don't really need this -->
```
Another limitation with this study is that logistic regressions assumes
that the data is binomial, which the data used is not. The analysis
assumes a linear correlation between the dependent variable and the
independent variable, which is best analysed in a normal distribution.
However, Fischers test showed a significant difference between the
groups in 4 of the 7 variables, all of them being the ones who showed an
increased or decreased risk of delayed treatment, showing that these
results still are significant.
...
results can be interpreted as that it is mostly injured and less awake
patients that receive delayed. The clinical applications for this are
not clear, but caregivers should already be aware that the more injuries
a patients has, the more room for error exists. It is good though to
receive a reminder that it's important to always work towards minimizing
the delay for these injured patients.
...
applicable to the whole trauma patient population. However, as it is a
single center study, where its result only illustrates the situation in
Stockholm, it is less applicable nationally or globally. This does not
make it only applicable to the population of Stockholm, but is something
to keep in mind.

```{=html}
<!-- Think about what components of your study is generalisible and to 
where. I think that your results are generalisible to  a large extent, in 
the sense that severly injured patients are at higher risk of delayed 
treatment, also in other contexts -->
```
#### Future studies
